...
const STEPS = [
  {
    key: "encrypting",
    label: "Encrypt position data",
    desc: "Size, side, and leverage encrypted client-side",
  },
  {
    key: "proving",
    label: "Generate Arcium proof",
    desc: "MXE verifies position validity privately",
  },
  {
    key: "ready",
    label: "Submit on-chain",
    desc: "Only commitment hash hits the blockchain",
  },
];
...
export function ProofStatus({ compact = false }: ProofStatusProps) {
    ...
    return (
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          {arciumStatus === "idle" && (
            <Shield size={11} className="text-arcium-400" />
          )}
          {arciumStatus === "encrypting" && (
            <Loader2 size={11} className="text-gold-400 animate-spin" />
          )}
          {arciumStatus === "proving" && (
            <Loader2 size={11} className="text-arcium-400 animate-spin" />
          )}
          {arciumStatus === "ready" && (
            <ShieldCheck size={11} className="text-emerald-400" />
          )}
          {arciumStatus === "error" && (
            <ShieldAlert size={11} className="text-crimson-400" />
          )}
        ...
        <button
          ...
          className="text-dim hover:text-ghost transition-colors"
        >
          <Info size={11} />
        </button>
      </div>
            ...
            return (
              <div
                ...
                className={`flex items-start gap-2 p-2 rounded-md transition-all duration-300 ${
                  isCurrentStep
                    ? "bg-arcium-500/10 border border-arcium-500/20"
                    : ""
                }`}
              >
                <div className="mt-0.5 shrink-0">
                  {isDone ? (
                    ...
                    <Loader2
                      size={14}
                      className="text-arcium-400 animate-spin"
                    />
                  ) : (
