...
export function PrivacyBadge({ compact = false }: { compact?: boolean }) {
  ...
  if (compact) {
    return (
      <div
        className={`inline-flex items-center gap-1.5 px-2 py-1 rounded border text-xs font-mono ${config.bg} ${config.color}`}
      >
  ...
  return (
    <div
      className={`flex items-center gap-2.5 px-3 py-2 rounded-lg border ${config.bg}`}
    >
      ...
      <div>
        <div className={`text-xs font-mono font-500 ${config.color}`}>
          {config.label}
        </div>
  ...
  return (
    <div className="panel p-4 space-y-3">
      ...
      <div className="space-y-2">
        {[
          ...
          <div key={row.label} className="flex items-center justify-between">
            ...
            <span
              className={`text-xs font-mono ${row.private ? "text-arcium-400" : "text-ghost"}`}
            >
      ...
      <p className="text-xs text-dim leading-relaxed pt-1 border-t border-border">
        Position data is encrypted via Arcium MXE. Liquidation checks run
        privately — liquidators learn only that a position is underwater, never
        the size or entry price.
      </p>
    </div>
  );
}
